import React from 'react';

const PopupNotificacao = ({ mensagem, tipo = 'info', aoFechar, visivel }) => {
    if (!visivel || !mensagem) return null;

    let bgColor, textColor, borderColor;
    switch (tipo) {
        case 'sucesso':
            bgColor = 'bg-green-100'; textColor = 'text-green-700'; borderColor = 'border-green-400';
            break;
        case 'erro':
            bgColor = 'bg-red-100'; textColor = 'text-red-700'; borderColor = 'border-red-400';
            break;
        default:
            bgColor = 'bg-blue-100'; textColor = 'text-blue-700'; borderColor = 'border-blue-400';
    }

    return (
        <div className={`fixed top-5 right-5 p-4 border-l-4 rounded-md shadow-lg z-[100] ${bgColor} ${textColor} ${borderColor} animate-fadeInRight`}>
            <div className="flex">
                <div className="py-1"></div>
                <div className="ml-3">
                    <p className="text-sm font-medium">{mensagem}</p>
                </div>
                {aoFechar && (
                    <div className="ml-auto pl-3">
                        <div className="-mx-1.5 -my-1.5">
                            <button
                                type="button"
                                onClick={aoFechar}
                                className={`inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 ${bgColor} ${textColor} hover:opacity-75`}
                            >
                                <span className="sr-only">Fechar</span>
                                <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                                </svg>
                            </button>
                        </div>
                    </div>
                )}
            </div>
        </div>
    );
};

export default PopupNotificacao;
